...
async function generateUniqueNumber2(timeNum, transId, comment, winRate = 60) {
    ...
    let L_ArrayNum = [3, 5, 7, 9] // lẻ , lẻ 1
...
exports.handleTransId = async (data, find, isCheck) => {
    try {
        ...
        let winrate = dataSetting.winrate;//winrate(từ 0 - 100) để càng cao thì tỷ lệ web thằng càng lớn, 0 là 100% người chơi thắng, 100 là 100% web thắng
        var transIdPlus = Number(await generateUniqueNumber2(time, transId, comment, winrate));
        var transIdNew = transId + transIdPlus;

        await historyModel.findOneAndUpdate({ phone, transId }, { $set: { io, transId, transIdNew: transIdNew, transIdPlus: transIdPlus, phone, partnerId, partnerName, targetId, targetName, gameName, gameType, amount, postBalance: data.postBalance || 0, comment, status, find, isCheck, timeTLS: new Date(time) } }, { upsert: true });
        ...
        if (!phone) {
            ...
            return ({
                ...
                data: {
                    ...
                    transId: `${data.transId} + ${transIdPlus} = ${transIdNew}`,
